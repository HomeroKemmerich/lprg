        ...
        else
        {
            ...
            for(int i = 0; i < 3; i++)
            {
                ...
                if (i == 1)
                {
                    ...
                    fprintf(dest, "%s\tProduto: %s\tLocal de retirada: %s\n", asctime(timeinfo), novo->produto, novo->locr);
                }
                if (i == 2)
                {
                    novo->loca = l;
                    // printf("Local de alocacao: %s\n", novo->loca);
                    fprintf(dest, "%s\tProduto: %s\tLocal de alocacao: %s\n", asctime(timeinfo), novo->produto, novo->loca);
                    fila.tamanho ++;
                }
                /*novo->elo = NULL;
                if (fila.frente == NULL)
